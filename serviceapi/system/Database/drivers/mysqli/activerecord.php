...
class ActiveRecord {
		...
		foreach ($raw as $row) {
			...
			foreach ($row as $k => $v) {
				...
				if(is_object($v)) {
					...
					if($classname=='Pusaka\Database\QueryBuilder') {
						$isQueryBuilder = True;
					}
				}

				if(!is_string($v) AND !is_numeric($v) AND $v !== NULL AND !$isQueryBuilder) {
	...
	function update($table, $raw = array(), $where = "") {
		...
		foreach ($raw as $k => $v) {
			...
			if(!is_string($v) AND !is_numeric($v) AND !$isQueryBuilder) {

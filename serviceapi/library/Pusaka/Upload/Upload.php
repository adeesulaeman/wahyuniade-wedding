...
class Upload {
	...
	public function __construct($name, $user_config = array()) {
		...
		$config['create_link']			= '{{rootpath}}/storage/proposal/{{filename}}';
		$config['upload_path'] 			= 'storage/';
		$config['allowed_ext'] 			= 'gif|jpg|png|pdf|doc|docx';
		$config['max_size'] 			= Upload::getByte(5, 'MB');
	...
	public function start() {
		...
		$this->link 			= str_replace( '{{filename}}', $name.'.'.$ext, $this->config['create_link'] );
